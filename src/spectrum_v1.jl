...
mutable struct Spectrum
    ...
    logfile_F   :: IOStream         # logfile F
end

function Spectrum(json_file)
    # create a Parameter object form the jso strinh
    par = Parameter(json_file)
    ...
    logfile_F = open(joinpath(par.out_dir, par.logfile_F), "w")

    # output of isotope ids
    #mut keys_vec: Vec<(&usize, &usize)> = isotope_ids.iter().collect();
    #keys_vec.sort_by(|a, b| a.0.cmp(b.0));

    ## infofile
    #mut infofile = open_file(par.out_dir.as_str(), par.infofile.as_str());
    ...
    #    Err(why) => { panic!("couldn't write to infofile: {}", why.to_string()) },
    #    Ok(_) => 0
    #};
    #print!("{}", joined);
    #infofile.flush();
...
function compute_emission_and_absorption(spec::Spectrum, T::Float64, N::Float64, p::Float64, NCO2::Float64)
    ...
    Threads.@threads for iline in 1:nb_lines
        ...
        #@infoe @sprintf("%12.5e %12.5e %12.5e %12.5e %12.5e %12.5e %12.5e %12.5e %12.5e %12.5e %12.5e %12.5e",
        #                γ, γ_s, γ_a, dT, NCO2, p, spec.par.T_ref, T, n_a,
        #                spec.lines[iline].ΔλL, spec.lines[iline].ΔλG, λ_ul - λ_ul0)
        ...
        spec.line_data.linesv[i_γ   ,iline] = γ
        spec.line_data.linesv[i_ΔλL ,iline] = ΔλL
        spec.line_data.linesv[i_ΔλG ,iline] = ΔλG
        spec.line_data.linesv[i_N_l ,iline] = N_l
        spec.line_data.linesv[i_N_u ,iline] = N_u
        spec.line_data.linesv[i_ϵ   ,iline] = ϵ
        spec.line_data.linesv[i_κ   ,iline] = κ
        spec.line_data.linesv[i_λ_ul,iline] = λ_ul
...
function integrate_along_path(spec::Spectrum, iN, iθ, NCO2, θ)
    # z
    nb_zsteps = size(spec.z,1)
    zp = spec.p_vs_h.xy[1, size(spec.p_vs_h.xy,1)]

    z_max  = spec.t_vs_h.xy[1, size(spec.t_vs_h.xy,1)]
    if z_max > zp
        z_max = zp
    end
    if z_max > spec.z[nb_zsteps]
        z_max = spec.z[nb_zsteps]
    end
    znext = 10.0
    Dz = 2.0e3
    ...
    # results array
    results = Vector{StaticVector{11, Float64}}(undef, 0)
    sizehint!(results, nb_zsteps)
    ...
    for istep in 1:nb_zsteps
        ...
        # sum_over_lines
        # compute ϵ(λ) = sum_i ϵ_i(λ_i) f(λ-λ_i)
        # compute κ(λ) = sum_i κ_i(λ_i) f(λ-λ_i)
        #@time
...
#        end
        ...
        ds = 1.0
        if istep < nb_zsteps
            ds = (spec.z[istep+1] - spec.z[istep]) / cos(θ)
        else
            ds = (spec.z[istep] - spec.z[istep-1]) / cos(θ)
        end
        ...
        for iλ in 1:nb_λ
            ...
            if spec.par.with_emission
                eps = 0.0
                if abs(κ) * ds < κds_limit
                    eps = ϵ * ds
                else
                    eps = ϵ / κ * (1.0 - exp_κ)
                end
        ...
        if spec.z_iout[istep] == 1
            znext = spec.z[istep] + Dz

            Iname = @sprintf("intensity_%d_%d_%5.3e", iN, iθ, spec.z[istep])
            save_intensity_as_hdf5(joinpath(spec.par.out_dir, Iname), spec.λ, I_λ)

            cname = @sprintf("spectrum_%d_%d_%5.3e", iN, iθ, spec.z[istep])
...
function integrate(spec::Spectrum)
    ...
    for (iN, NCO2) in enumerate(vNCO2)
        ...
        for (iθ, θ) in enumerate(vθ)
            fname = joinpath(spec.par.out_dir, @sprintf("result_%d_%d.hdf5", iN, iθ))
            ...
            result_data = integrate_along_path(spec, iN, iθ, NCO2, θ)
